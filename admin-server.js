...
// Normalize any date input to DD-MM-YYYY for TEXT column storage.
// Date column is TEXT type — stores dates in DD-MM-YYYY format.
function normalizeDateForDB(value) {
  if (!value) return value;
  const s = String(value).trim();
  // Already DD-MM-YYYY — pass through
  if (/^\d{2}-\d{2}-\d{4}$/.test(s)) return s;
  // YYYY-MM-DD (from date picker) → convert to DD-MM-YYYY
  const iso = s.match(/^(\d{4})-(\d{2})-(\d{2})$/);
  if (iso) return `${iso[3]}-${iso[2]}-${iso[1]}`;
  // DD-Mon-YY (legacy format)
